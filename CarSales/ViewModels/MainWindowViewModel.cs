...
namespace CarSales.ViewModels
{
    public partial class MainWindowViewModel : ObservableObject
    {
        ...
        [ObservableProperty]
        private string _newNetPrice = string.Empty;

        [ObservableProperty]
        private DateTime? _newSoldOn = DateTime.Today;

        [ObservableProperty]
        private string _newVatPercent = string.Empty;
        ...
        [RelayCommand]
        private void AddVehicleRecord()
        {
            double netPrice = 0d, vatPercent = 0d;
            try
            {
                if (!double.TryParse(NewNetPrice, out netPrice))
                    throw new ArgumentException($"Failed to parse Net price '{NewNetPrice}' to double.");

                if (!double.TryParse(NewVatPercent, out vatPercent))
                    throw new ArgumentException($"Failed to parse Vat percent '{NewVatPercent}' to double.");
            }
            ...
            var manufacturer = _salesData?.Manufacturers
                .FirstOrDefault(m => m.Name.Equals(NewManufacturer, StringComparison.OrdinalIgnoreCase));

            if (manufacturer == null)
            {
                manufacturer = new Manufacturer() { Name = NewManufacturer };
        ...
        private void RecalculateStatistics()
        {
            ...
            if (SelectedTabIndex == 0) // All vehicles
            {
